...
#define DIE(...) do { fprintf(stderr, __VA_ARGS__); perror(""); exit(1); } while(0)
...
{
	struct incrementer_input *input = (struct incrementer_input*)void_input;
...
int main(int argc, char **argv)
{
	...
	struct incrementer_input *inputs = malloc(sizeof(struct incrementer_input) * SEGMENTS);
