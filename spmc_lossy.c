/* single producer, multiple consumer queue which throws away
   ...
   Improvements this could brook:
     ...
     * signal handler to respond to SIGINT properly
     * probably isn't actually correct
*/
...
static int open_queue(queue *q)
{
	...
	void *region =
	    mmap(NULL, QUEUE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
static void enqueue_line(queue *q, const char *begin, const char *end)
{
	...
	DEBUG("enq %u bytes (%u), fst=%u, last=%u\n", line_bytes, bytes_needed,
	      h->first_item_location, h->last_item_location);
	if (bytes_needed > h->bytes_available) {
		fprintf(stderr, "Can't enqueue line with %u bytes (max=%u)!\n",
		        line_bytes, max_line_len);
		goto enqueue_exit;
	}
	uint new_loc;
	...
	}
	{
		...
		assert(last->next == 0);
		uint after_last =
		    h->last_item_location + last->length + sizeof(q_item);
		...
		last->next = new_loc;
		while ((new_loc <= h->first_item_location) &&
		       (new_end > h->first_item_location) &&
...
static int create_queue(void)
{
	...
	struct q_head *head;
	int fd =
	    open(QUEUE_FILENAME, O_RDWR | O_CREAT | O_TRUNC | O_EXCL, 0600);
	if (fd < 0) {
