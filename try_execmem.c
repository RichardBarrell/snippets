...
int main() {
	...
	if (errno) { perror("adding write> "); }
	mprotect(m, 4096, PROT_READ);
	if (errno) { perror("removing write> "); }
	mprotect(m, 4096, PROT_READ|PROT_WRITE|PROT_EXEC);
	if (errno) { perror("adding write+exec> "); }
	munmap(m, 4096);
	if (errno) { perror("munmap> "); }
	...
	if (errno) { perror("> "); }
	mprotect(m, 4096, PROT_READ|PROT_WRITE|PROT_EXEC);
	if (errno) { perror("adding exec> "); }
	munmap(m, 4096);
	if (errno) { perror("munmap> "); }

	m = mmap(NULL, 4096, PROT_READ|PROT_EXEC, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
	...
	m = mmap(NULL, 4096, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
