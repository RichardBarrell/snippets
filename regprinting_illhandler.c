/* This is a really evil C program that installs a SIGILL handler in order to make the
   "opcodes" 0x06 and 0x07 (which normally just make your program crash) dump the
   current CPU registers on stderr.

   This blithely assumes that nothing else is going to try to use SIGILL for anything,
   which is obviously true because only a LUNATIC would invoke SIGILL on purpose, right?
...
static void regprinting_illhandler(int signum, siginfo_t *info, void *ucontext) {
	...
	size_t regdumpsz = snprintf(regdump, 1024,
	                            "rax      = %#018lx\n"
	                            "rbx      = %#018lx\n"
	                            "rcx      = %#018lx\n"
	                            "rdx      = %#018lx\n"
	...
	if (regdumpsz < 1024) {
		/* I call write(2, ...) and don't check the result because there isn't anywhere
		 * sensible to which I could log a failure to write to stderr anyway, and
	...
	if (*manip == 0x06) {
		/* What we're doing here is incrementing the instruction pointer. When the signal
		 * handler returns, control resumes at the next, probably legal, instruction. */
		gregs[16] += 1;
		const char skipped[] = "Skipped 0x06.\n";
		write(2, skipped, sizeof(skipped) - 1);
	} else if (*manip == 0x07) {
		/* This makes many unportable assumptions about how memory works.
		...
		if (mprotect(ppage, (size_t)pagesz, PROT_READ|PROT_WRITE|PROT_EXEC)) {
			const char mprotect_write_fail[] = "Can't make page writable.\n";
			write(2, mprotect_write_fail, sizeof(mprotect_write_fail) - 1);
		}
		*manip = 0x90;
		if (mprotect(ppage, (size_t)pagesz, PROT_READ|PROT_EXEC)) {
			const char mprotect_unwrite_fail[] = "Can't make page unwritable.\n";
			write(2, mprotect_unwrite_fail, sizeof(mprotect_unwrite_fail) - 1);
		}
		const char patched[] = "Patched 0x07.\n";
		write(2, patched, sizeof(patched) - 1);
	} else {
		/* Not one of our two magic bytes? Must have caught a non-deliberate ILL! */
		abort();
	}
}
