...
int main(int argc, char **argv)
{
    ...
    if (read_int_file("/sys/class/backlight/gmux_backlight/max_brightness", &max)) {
        fprintf(stderr, "Can't read max brightness.\n");
        return 1;
    }
    if (read_int_file("/sys/class/backlight/gmux_backlight/brightness", &cur)) {
    ...
    if (write_int_file("/sys/class/backlight/gmux_backlight/brightness", level)) {
